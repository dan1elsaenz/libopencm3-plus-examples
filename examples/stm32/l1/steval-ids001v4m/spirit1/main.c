...
Data_write transmit_conf_data[] = {
    ...
    {0x27, 0x15}, // RX FIFO filling depends on Carrier Sense threshold
    ...
    {0x50, 0x46}, // Start calibration. TODO: check calibration is finished
};
...
static void leds_init(void) {
  ...
  gpio_mode_setup(LEDS_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, GPIO0 | GPIO1);
}
...
int main(void) {
  ...
  //  wait(10);
  printf("\n");
  status = sp1_read(spsgrf_spi, SP1_ANA_FUNC_CONF, rx_values, 2, true);
  ...
  printf(
      "(r) read, (w) write, (c) cmd, (s) get status, (b) read&print buffer\n");
  while (true) {
    if (poll(stdin) > 0) {
      ...
      switch (my_cmd[0]) {
      case 'r':
        printf("Enter register address to read and number of readings: "
               "reg_addr count \n");
        read_serial(my_input);
        ...
        status = sp1_read(spsgrf_spi, sp1_reg, rx_values, read_counts, true);
        printf("--------\n");
      ...
      case 'w':
        ...
        printf("\n");
        printf("--------\n");
        status = sp1_write(spsgrf_spi, sp1_reg, write_data, write_counts);
        break;
      case 'c':
      ...
      case 'b':
        ...
        status =
            sp1_read(spsgrf_spi, SP1_LINEAR_FIFO_STATUS, rx_values, 2, true);
